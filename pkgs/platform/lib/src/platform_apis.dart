...
/// @docImport 'testing/test_platforms.dart';
...
// Helper to allow a `const factory` constructor to return
// the same value every time.
extension type _PlatformHelper._(Platform _) implements Platform {
  const _PlatformHelper() : _ = impl.platformInstance;
}
...
abstract final class BrowserPlatform {
  ...
  ///
  /// Same as [BrowserPlatform.current][Platform.browserPlatform].
...
// Base classes that are only used internally, by the implementation
// or test classes.

abstract base class PlatformBase
    with LegacyPlatformMembers
    implements Platform {
  const PlatformBase();
}

abstract base class NativePlatformBase implements NativePlatform {
  const NativePlatformBase();
}

abstract base class BrowserPlatformBase implements BrowserPlatform {
  const BrowserPlatformBase();
}
