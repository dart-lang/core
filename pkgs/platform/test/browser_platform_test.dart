...
import 'dart:js_interop'; // For `appVersion` and `navigator`.
...
void main() {
  ...
  test('Has same values as JS', () {
    ...
    expect(browserPlatform.userAgent, userAgent,
        reason: 'window.navigator.userAgent');
    expect(browserPlatform.version, appVersion,
        reason: 'window.navigator.appVersion');
  });
}

@JS('window.navigator.appVersion')
